...
use Laravel\Horizon\Http\Controllers\CompletedJobsController as BaseCompletedJobsController;

class CompletedJobsController extends BaseCompletedJobsController
{
    /**
    ...
    protected function searchByName(Request $request, string $query): array
    {
        ...
        while ($matched->count() < $limit && $iterations < $maxIterations) {
            $batch = $this->jobs->getCompleted($cursor);
