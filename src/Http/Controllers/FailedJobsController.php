...
use Laravel\Horizon\Http\Controllers\FailedJobsController as BaseFailedJobsController;

class FailedJobsController extends BaseFailedJobsController
{
    /**
    ...
    protected function searchByName(Request $request, string $query): array
    {
        ...
        while ($matched->count() < $limit && $iterations < $maxIterations) {
            $batch = $this->jobs->getFailed($cursor);
        ...
        $jobs = $matched->take($limit)->map(fn ($job) => $this->decode($job))->values();
